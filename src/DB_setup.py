...
def insert_data(engine, cleaned_df):
    ...
    try:
        ...
        cleaned_df["message_date"] = cleaned_df["message_date"].apply(
        ...
        with engine.begin() as connection:  # Auto-commit enabled
            for _, row in cleaned_df.iterrows():
                ...
                logging.info(
                    f"Inserting: {row['message_id']} - {row['message_date']}")
        ...
        logging.info(
            f" {len(cleaned_df)} records inserted into PostgreSQL database.")
    except Exception as e:
